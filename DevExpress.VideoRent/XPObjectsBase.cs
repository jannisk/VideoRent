...
namespace DevExpress.VideoRent {
    public class InitialRecords {
        public static void CheckForInitialRecords(Session session) {
            using(var saver = new UnitOfWork(session.DataLayer)) {
                saver.LockingOption=LockingOption.None;
        ...
        static void CheckForDefaultCategories(Session session) {
            ...
            var category = new MovieCategory(session, Properties.Resources.DefaultMovieCategoryName);
        ...
        static void CheckForCompanyTypes(Session session) {
            ...
            foreach(var name in ReferenceData.CompanyTypes)
        ...
        static void CheckForMovieLanguages(Session session) {
            ...
            foreach(var name in ReferenceData.MovieLanguages)
    ...
    public class VideoRentCurrentUser {
        ...
        public static bool Login(Session session, string login, string password) {
            ...
            var administrator = session.FindObject<Employee>(CriteriaOperator.Parse("Login = ? And Password = ?", login, StringEncoder.CalcHash(password)));
        ...
        public static Employee GetCurrentUser(Session session) {
            ...
            return  session.FindObject<Employee>(CriteriaOperator.Parse("Oid = ?", currentUserOid));
            //<Employee>(currentUserOid);
        }
    }
    ...
    public class Person : VideoRentBaseObject {
        ...
        public virtual string FullName {
            get {
                var ret = FirstName == null ? string.Empty : FirstName.Trim();
                var lastNameTrim = LastName == null ? string.Empty : LastName.Trim();
