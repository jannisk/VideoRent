...
namespace DevExpress.VideoRent.Wpf.Helpers {
    ...
    public class EventCommand : DependencyObject {
        ...
        static EventCommand() {
            var ownerType = typeof(EventCommand);
        ...
        public void Execute(object sender, EventArgs e) {
            ...
            var parameter = UseEventArgsInsteadOfParameter ? e : Parameter;
            var action = Command as Action<object>;
            if(action != null) {
                action(parameter);
                return;
            }
    ...
    public class EventCommandHelper : DependencyObject {
        ...
        static EventCommandHelper() {
            var ownerType = typeof(EventCommandHelper);
        ...
        public static ObservableCollection<EventCommand> GetEventCommands(DependencyObject d) {
            var eventCommands = (ObservableCollection<EventCommand>)d.GetValue(EventCommandsProperty);
        ...
        static void RaiseEventCommandsChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
            var oldValue = (ObservableCollection<EventCommand>)e.OldValue;
            var newValue = (ObservableCollection<EventCommand>)e.NewValue;
            if(oldValue != null)
        ...
        static void OnEventCommandsChanged(DependencyObject d) {
            foreach(var eventCommand in GetEventCommands(d)) {
                SubscribeToEvent(d, eventCommand);
            }
        }
        static void SubscribeToEvent(DependencyObject d, EventCommand eventCommand) {
            var eventInfo = d.GetType().GetEvent(eventCommand.Event);
            if(eventInfo == null) return;
            var methodInfo = typeof(EventCommand).GetMethod("Execute", new Type[] { typeof(object), typeof(EventArgs) });
            var delegateObject = Delegate.CreateDelegate(eventInfo.EventHandlerType, eventCommand, methodInfo);
        ...
        static bool CheckRowAndExecute(object owner, EventArgs e) {
            var view = owner as GridViewBase;
            var mea = e as MouseEventArgs;
            if(view == null || mea == null) return false;
            var rowHandle = view.GetRowHandleByMouseEventArgs(mea);
    ...
    public class CustomEventCommandHelper : EventCommandHelper {
        ...
        protected static void BindCommand(DependencyObject d, string eventName, DependencyProperty commandProperty, DependencyProperty parameterProperty) {
            var eventCommands = GetEventCommands(d);
            var eventCommand = new EventCommand() { Event = eventName };
